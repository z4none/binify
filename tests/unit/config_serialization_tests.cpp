...
TEST(ConfigSerializationTests, SerializesConfigToExpectedJsonFields) {
  ...
  EXPECT_NE(json.value().find("\"language\": \"system\""), std::string::npos);
...
TEST(ConfigSerializationTests, RoundTripsUnicodePath) {
  ...
  EXPECT_EQ(config.value().language, L"system");
  EXPECT_TRUE(config.value().context_menu_enabled);
  EXPECT_TRUE(config.value().path_enabled);
}

TEST(ConfigSerializationTests, DeserializesExplicitLanguage) {
  const auto config = deserialize_config_from_json(
    R"({"configVersion":1,"binDirectory":"C:\\bin","language":"zh-CN","contextMenuEnabled":false,"pathEnabled":false})");

  ASSERT_TRUE(config);
  EXPECT_EQ(config.value().language, L"zh-CN");
}

TEST(ConfigSerializationTests, DeserializesOldConfigWithoutLanguageAsSystem) {
  const auto config = deserialize_config_from_json(
    R"({"configVersion":1,"binDirectory":"C:\\bin","contextMenuEnabled":false,"pathEnabled":false})");
...
TEST(ConfigSerializationTests, RejectsInvalidLanguageCode) {
  Config config = sample_config();
  config.language = L"zh_CN!";

  const auto json = serialize_config_to_json(config);

  ASSERT_FALSE(json);
  EXPECT_EQ(json.error().code, ErrorCode::config_invalid);
}
