...
UninstallWindow::UninstallWindow(RuntimeContext& runtime) : runtime_(runtime) {
  window_title_ = runtime_.text("uninstall.title");
...
void UninstallWindow::create_controls() {
  ...
  title_label_.create(this, -1, runtime_.text("uninstall.heading").c_str(), {s(24), s(18)}, {s(420), s(34)});
  apply_font(title_label_.hwnd(), theme_.title_font());
  make_transparent_control(title_label_.hwnd());
  summary_label_.create(
    this,
    -1,
    runtime_.text("uninstall.summary").c_str(),
    {s(44), s(86)},
    {s(540), s(54)});
  apply_font(summary_label_.hwnd(), theme_.body_font());
  make_transparent_control(summary_label_.hwnd());
  path_checkbox_.create(this, kIdPath, runtime_.text("uninstall.remove_path").c_str(), {s(44), s(184)}, {s(500), s(26)});
  apply_font(path_checkbox_.hwnd(), theme_.body_font());
  make_transparent_control(path_checkbox_.hwnd());
  context_menu_checkbox_.create(this, kIdContextMenu, runtime_.text("uninstall.remove_context_menu").c_str(), {s(44), s(222)}, {s(500), s(26)});
  apply_font(context_menu_checkbox_.hwnd(), theme_.body_font());
  make_transparent_control(context_menu_checkbox_.hwnd());
  const auto cleanup_text = L"🗑  " + runtime_.text("uninstall.cleanup");
  cleanup_button_.create(this, kIdCleanup, cleanup_text.c_str(), {s(410), s(302)}, {s(110), s(36)});
  apply_font(cleanup_button_.hwnd(), theme_.body_font());
  make_modern_button(cleanup_button_.hwnd(), ButtonRole::danger);
  cancel_button_.create(this, kIdCancel, runtime_.text("common.cancel").c_str(), {s(534), s(302)}, {s(82), s(36)});
...
void UninstallWindow::run_cleanup() {
  ...
  std::wstring message = runtime_.text("uninstall.completed");
  message += cleaned.value().path_removed ? L"\r\n" + runtime_.text("uninstall.path_removed") : L"\r\n" + runtime_.text("uninstall.path_unchanged");
  message += cleaned.value().context_menu_removed ? L"\r\n" + runtime_.text("uninstall.context_menu_removed") : L"\r\n" + runtime_.text("uninstall.context_menu_unchanged");
